...
interface UseLocalOmniReturn {
  isConnected: boolean;
  isConnecting: boolean;
  transcript: string;        // translated text
  sourceTranscript: string;  // source text
  volume: number;
...
export const useLocalOmni = (): UseLocalOmniReturn => {
  ...
  // Play audio chunk with scheduled buffering (same pattern as useGeminiLive)
  ...
  const connect = useCallback(async (options?: { targetLanguage?: string }) => {
    ...
    try {
      ...
      const ws = new WebSocket(`${backendUrl}/ws/omni`);
      wsRef.current = ws;
      ...
      ws.onmessage = (event) => {
        ...
        switch (msg.type) {
          case 'source_text':
            setSourceTranscript(prev => prev + msg.data + ' ');
            break;
          ...
          case 'error':
            ...
            break;
          case 'status':
            // Could display processing status if needed
            break;
        }
      };
      ...
      ws.onerror = (err) => {
        console.warn('Local Omni WebSocket error:', err);
